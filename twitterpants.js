...
const envName = process.env.envName;// your environment name 
...
const getTweets = options => {
  //so basically, we sample each of the past 20 days for Tweets from each.
  for (let i = 1; i < 21; i++) {
    //Note that we do not wait for one response to come in before firing the next request
    ...
    if (from < 10) {
      //a little bit of housekeeping - for numbers 1-9, we want them to read 01 through 09
      from = "0" + from;
    }
    if (to < 10) {
      to = "0" + to;
    ...
    newOptions.url +=
      "&fromDate=201806" + from + "0000&toDate=201806" + to + "0000"; //we add date params to our existing url
    throttledRequest(newOptions, (error, response, body) => {
      ...
      body.results.forEach(tweet => {
        tweet.text = tweet.text
          .replace(/(\r\n|\n|\r)/gm, "")
          .replace(/,/g, ";"); //remove newline characters and commas from the tweet text
        builderString +=
          tweet.id_str +
          "," +
          tweet.user.name +
      ...
      appendCSV("twitterpants.csv", builderString)
        .then(content => {
          //we can call appendCSV asynchronously - we don't have to wait on the results of the writing to the CSV to make our next API call
...
writeCSV(
  "twitterpants.csv",
  "id_str,username,favorite_count,retweet_count, text\n"
)
  .then(content => {
    //create the CSV first, with a line of headers
